...
export default function Input({token}){
    ...
    function addInput(e){
        setDesabilitar(true)        
        e.preventDefault()
        const requisicao = axios.post("http://localhost:5000/entrada", { description: description, value: valor.replace(/[,]/g, ".") }, { headers: { "Authorization": `Bearer ${token}` }, })
    ...
    return(
        ...
        <ContainerInput>
            <form onSubmit={addInput}>
                ...
                <SalvarEntrada type="submit">Salvar entrada</SalvarEntrada>
            </form>                
            </ContainerInput>
        </>
    )
